...
**Cited, machine-checkable change detection for US transgender identity-document law and process.** It watches registry-claimed government-source candidates — state vital records, DMVs, courts, State Department, SSA, the Federal Register — on a polite weekly cadence. For HTML and plain text, it hashes normalized text and produces the **passage that changed**; for a PDF it does the same from the document's own extracted page text, or says plainly that it could not read the document rather than guessing at it. A named human reviews every detected change before it is published. The tool reports an observation about a URL; it never asserts what the law is or that an unverified URL is authoritative.
...
[`docs/00-V1-PLAN.md`](./docs/00-V1-PLAN.md) indexes the V1.0 plan, written
July 13, 2026, as a forward-looking open-source plan.

**V1.0 means:** every active source is human-verified; high-impact publication has independent review; the feed contract and correction path are stable; eight weekly watch→review→publish cycles satisfy the pre-release evidence rule; accessibility evidence is complete; and every must-pass gate in [`docs/15-V1-RELEASE-CHECKLIST.md`](./docs/15-V1-RELEASE-CHECKLIST.md) has an owner and dated receipt. V1.0 does not mean the service interprets law, gives advice, or guarantees it observed every policy change.
...
The fix is not a disclaimer, it is the work: **`sentinel verify`** is the review aid that makes it cheap — it fetches each source, shows a human the page's own title and an excerpt of its text, asks one question, and records the answer **with the verifier's name and the date**. It refuses to record a verification without a name, and the registry will not even *load* a `verified: true` entry that has no human behind it. See [`docs/VERIFYING.md`](./docs/VERIFYING.md) — it is about three and a half hours of work for all 156 sources, and it is the most valuable three and a half hours anyone could spend on this repo. **Verification is one of two decisions a source needs before it is ever fetched.** The other is a named human's dated reading of the host's robots.txt and terms, recorded with `sentinel sources policy`; a source with only one of the two stays out of the attempt denominator, and both commands say which is missing rather than letting an afternoon's work end in an empty feed.
...
uv run sentinel sources rotation   # …and the ones that started rotating AFTER (no network)
...
- **Tells you what changed — for text/HTML, and now for a PDF it can read.** On text or HTML drift it computes a unified diff of the **normalized text** and hands the reviewer the changed passages. A PDF gets the same treatment from its extracted page text, so a re-render with a new build date and a form that now demands a court order stop looking identical in the alert.

  **Detection did not move to the extracted text, and that is the safety property.** A PDF's hash still covers the whole file, so a change in bytes the extractor does not read — an annotation, an image, embedded metadata — still produces a change record; the excerpt then says the page text did not move, and explicitly does not say the document is unchanged. **A PDF outside the readable subset is refused as a whole rather than read in part**, because a diff missing the half that changed is worse than no diff: the reviewer reads "nothing changed here" and is wrong. Every refusal carries a reason, so the documents this subset cannot read are countable rather than assumed — `sentinel sources check` prints it per source.

  **`--twice` is necessary and not sufficient, and the other half now has a command too.** It catches per-*request* rotation. A widget that re-rolls hourly, daily, or per cache generation passes it cleanly and drifts week over week anyway, and a page that *starts* rotating after it was registered is not covered by a registration-time check at all. What notices those is a reviewer dismissing the same source as `editorial` week after week — until now a signal that lived in one person's memory. `sentinel sources rotation`, printed automatically at the end of every watch, reads it out of the review record instead. **It suppresses nothing and says so:** the flagged source is still watched, still produces a change record on the next real edit, and the report explicitly refuses to choose between "this page churns" and "this page is genuinely edited every week". Muting it would be the wrong "no change" arriving as a noise-reduction feature.
...
- **Commits the baseline, so a clean clone has a memory without bypassing source policy.** The snapshot store is not committed (it is megabytes of government HTML and grows weekly), which used to mean a fresh checkout knew nothing. `sources/baseline-hashes.json` retains 145 historical hashes, but `sentinel baseline check` now fetches only sources that pass the same dated verification and fetch-policy predicate as `sentinel watch`. From a fresh clone it can answer *"which eligible page is not what it was?"* without a store. Today that means zero network requests because the honest attempt denominator is zero. It holds the prior hash, not the text, so after sources become eligible it can say that a page moved but not what moved; `sentinel watch` retains the evidence needed to show the passage.
...
- **Cannot lie about its own coverage — and the gate's own scope is stated, not implied.** These figures are *derived* and never typed by hand: sources, jurisdictions, named gaps, unreachable, human-verified (all from `sources/registry.json`, by `sentinel coverage`), and the historical hashes the committed baseline retains (from `sources/baseline-hashes.json`). `sentinel coverage --check-docs` re-derives them and fails the build if any gated document states one of them differently, if this README drops any of the five coverage numbers it is required to carry, or if a jurisdiction/document-class pair is neither watched nor a **named gap**. **What it does not do:** it reads one fixed set of phrase shapes, so a figure written in any other shape — a duration, a percentage, a jurisdiction count narrated as history — is outside it and is maintained by hand. This bullet used to claim *"every number in this README"* was derived; it was not, and three hand-maintained numbers on this page had gone stale underneath the claim. A project whose pitch is *"we tell you what went stale"* cannot have a stale front page, and cannot overstate the machine that is supposed to prevent one. The gate found two silent holes on the day it was written (see below).
...
| Quality & Metrics | Applies | 90% branch-coverage floor, enforced by `make cov` inside the literal `make verify`; coverage numbers in docs are re-derived from the registry by a merge-blocking gate. The floor is stated rather than the measurement: the measured percentage moves with every test added, nothing re-derives it into these docs, and a stale *"measured N%"* is a claim no build would ever correct — `make cov` prints today's figure |
