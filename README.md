...
**Current alpha limits:** PDF and other binary changes do not yet have extracted-text or passage diffs; the SQLite store keeps only the newest **five** snapshots per source and prunes older ones; and neither durable long-term reproducibility nor full WCAG 2.2 AA conformance is a current claim. Both are V1 release gates. The canonical source-eligibility evaluator is now enforced by both the watcher and publisher. Each watch persists the exact eligible, attempted, and successful source-ID sets plus its registry revision; `status.json` and the site distinguish quiet, partial, failed, running, and stale health from page generation time. Because the committed registry still needs real human verification evidence/expiry and dated robots/terms/fetch-policy decisions, its honest attempt denominator is currently **zero** and `sentinel watch` fails closed. No decision is inferred to make a dashboard look green.
