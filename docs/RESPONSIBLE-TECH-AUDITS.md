...
This is also why the published site makes **no third-party request at all**: the hosting logs are a risk we cannot remove, so we refuse to *add* a second one. A CDN font on the site would hand a third party the same list, for a nicer typeface.
