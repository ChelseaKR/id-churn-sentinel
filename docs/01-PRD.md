...
- For each pilot partner independently, achieve a median paired research-time reduction of at least 50% across its eight predeclared matched investigations; never pool partners or discard an observed pair after results are known.
...
| PILOT-01 | Design-partner validation | Two partners complete the six-week protocol; each completes eight predeclared matched investigations, independently reaches a median paired time reduction ≥50%, and answers ≥8/10 fixed comprehension tasks correctly; one adopts a standing integration; safety thresholds pass. No partners or observations are pooled or excluded post hoc. |
...
Territories, international documents, browser-rendered sources, partner-hosted private mirrors, source relationship graphs, richer review UI, and additional document classes. None may enter V1 if it delays P0 evidence.

## Metrics

**Leading:** verified-source burn-down; pilot recruitment; median diff review minutes; false-drift dismissal rate; stale-source count; weekly cycle completion; schema conformance; remediation aging.

**Lagging:** partner research-time reduction; number of partner content updates initiated by the feed; partner retention; missed-change discoveries; correction rate; incidents; sustainable cost per watched source.
...
- Can a partner fund the common public good without gaining editorial influence?
