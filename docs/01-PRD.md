...
| PRIV-01 | Data minimization | No accounts, subscriber list, pixels, or sensitive search logging exist in the public product, and no feed or data file carries analytics, cookies or third-party resources. The one exception is the owner-decided Google Analytics 4 page count on the two web pages ([ADR 0004](./adr/0004-count-page-visits-with-ga4.md)), which sends no query string, fragment, typed text or user identifier and does not run under GPC, DNT or opt-out. |
