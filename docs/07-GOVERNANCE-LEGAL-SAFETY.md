...
This document describes the proposed V1 governance model. The decision bodies
and reviewer roles below are design targets, not bodies that exist today.
...
| Privacy | what laws/contracts govern reviewer contacts and host logs? | data map, retention schedule, vendor terms, incident notice matrix, no end-user data |
| Accessibility | which federal/state obligations apply? | WCAG 2.2 AA is baseline; document audit and accommodation channel |
| Defamation / institutional harm | could alerts falsely accuse agencies or advocacy organizations? | factual observation language, evidence citation, correction/right-of-reply path, no grading organizations |
...
Must understand the evidence model, noise patterns, and non-advice boundary. They decide whether an observed change deserves downstream attention and document uncertainty.
...
Publish any funders and material organizational relationships. A funder cannot suppress an observation, choose classifications, accelerate its own jurisdiction, or obtain reader data. Reviewers disclose agency or organizational employment related to an item and recuse where independence is impaired.
