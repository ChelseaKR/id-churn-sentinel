...
The present implementation already separates observed drift from named human review and publishes static, anonymous artifacts. V1 needs a clear boundary that determines which roadmap, architecture, business, and release work belongs.
...
V1 does not interpret law, advise individuals, auto-classify legal significance, infer identity, collect reader data, promise exhaustive detection, evade source restrictions, or replace an advocacy/legal-aid guide. The public open feed is the common factual layer; paid work may provide integration and service levels but cannot influence observations or reveal readers.

## Consequences

### Positive

- reduces the narrow freshness burden while preserving partner expertise and trust;
...
- positions incumbent guidance providers as customers and collaborators.
...
- commercial value must come from institutional workflow and reliability, not reader growth or proprietary legal content.
...
Revisit only if design-partner evidence shows the institutional freshness job is not valuable, a trusted incumbent assumes the capability, or legal/governance review finds the boundary still creates unacceptable harm. Expanding into guidance requires a separate product, governance model, legal review, data design, and ADR; it is not an incremental V1 feature.
