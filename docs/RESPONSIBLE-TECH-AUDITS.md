...
- **The registry covers 52 of 52 jurisdictions**, and **8 named gaps** remain — each a (jurisdiction, document class) pair we do not watch, with the host that refused us and the reason. The feed's silence about any of them means nothing at all.
...
- **12 of the 156 registered sources cannot currently be fetched**, and are therefore *watched in name only*. This is a measured figure, not an estimate: an end-to-end `sentinel sources check` over the live registry on 2026-08-22 read **144 of 156**. `ssa.gov` (both federal SSN sources) returns 403 to every client we have; `health.ny.gov` returns 200 to a browser and 403 to our User-Agent; `cdph.ca.gov` ships a broken TLS chain; `ilsos.gov` will not complete an HTTP exchange at all; and, as a regression found 2026-08-21 and reconfirmed on 2026-08-22, `travel.state.gov` (both federal passport sources) sits behind a Cloudflare bot-challenge, `ldh.la.gov` (both Louisiana vital-records sources) returns a blanket 403 including on its own `/robots.txt`, `legislature.mi.gov`'s two MCL PDFs fail TLS verification (an incomplete chain a browser silently repairs and this fetcher will not), and `courts.mo.gov` returned HTTP 500 on the pass and again on an independent retry. Each is named in `sources/registry.json` with the machine facts. **They are not one kind of failure, and the count deliberately does not pretend otherwise:** seven are refusals, three are TLS failures, one is a timeout, and one is a server-side outage — and a 5xx is emphatically not a block, so it is no reason to demote a source to a named gap. **The evidence behind them is not uniform either:** five carry **no baseline hash** — a hash we did not observe is not a hash — while seven still hold the last hash actually observed, dated to when it was observed. No kind is ever reported as *unchanged*: a fetch that fails is reported as a failure on every run it recurs, because "we could not look" and "we looked and nothing moved" are the two sentences this project exists to keep apart. They are not deleted, because deleting them would erase the fact that we cannot watch them.
- **One source moved, and the move is confirmed without the reachability being oversold.** `nycourts.gov`'s name-change page: the 2026-07-13 URL returns 404, and the entry now points at the path NY Courts migrated CourtHelp to, which the full pass read cleanly — HTTP 200, *"Name Change Basics | New York Courts"*, 222 passages, on-topic for both name change and sex-designation change. On the same day, 5 consecutive targeted attempts (4 spaced two minutes apart) all returned 403. The difference is request spacing: the full pass reaches the host once, after a long gap. So the entry records `reachable: true` **with an explicit `intermittent` flag**, rather than either cherry-picking the success or writing off a source that demonstrably serves. A 403 on a later run is an outage and is reported as one.
- **The feed is currently empty, and an empty feed is not a claim.** No change has been reviewed and confirmed, so nothing is published. `docs/feed.xml` says so in-band (an XML comment: *"EMPTY, not broken … not a claim that nothing changed at any watched source"*), because the two failure modes of a blank feed are a consumer who thinks it is broken and stops reading, and a consumer who reads the silence as reassurance. The second is a wrong "no change" with extra steps.
...
**That other half is now read out of the record instead of out of a reviewer's memory (2026-09-05).** `sentinel sources rotation` — printed automatically at the end of every `watch` — names each source whose most recent reviewed observations a named human kept dismissing as `editorial`, with the streak, the elapsed span, the change ids, and who signed them. The threshold is the two this section already committed to, stated as a policy in units of reviewed observations rather than dressed up as a measurement.

**The version of this that would belong in this section as a harm is the one that acts on its own conclusion.** Muting a "rotating" source, weighting it down, or teaching the normalizer to strip the passage that keeps moving would each hide a real policy change behind a heuristic — on a page a reviewer has already been trained to ignore, which is the worst possible place to put a silent failure. So the detector reports and stops. A flagged source is watched on exactly the same terms as any other and still produces a change record on the next real edit, asserted by a test that exists for that purpose; the report states in band that nothing was suppressed, because an operator who believed otherwise would have been handed the wrong "no change" by a feature sold as noise reduction. It also refuses to choose between the two readings — a page that churns and a page that is genuinely edited every week look identical from the queue — and it classifies nothing, which `make no-auto-classification` now covers directly.
...
**A gap that was closed (2026-09-05), and the way it was nearly closed wrong is the point: PDF text extraction.** A PDF source could report one thing — that its bytes changed — so a re-render with a new build date and a form that now demands a court order produced the identical alert. `core/pdf.py` now extracts the page text, and the diff is computed over it.

The version of this feature that would have belonged in this section as a *harm* rather than a fix is the obvious one: **hash the extracted text.** Every change the extractor cannot see — an annotation appearance, an embedded image, page metadata — would then have stopped producing a change record at all. That is a wrong "no change" manufactured by an improvement, and it would have been invisible, because the feature would have looked like it was working. So detection stays on the raw bytes and extraction supplies only the diff. The bytes strictly contain the text, so a page edit cannot escape the hash, while an edit outside the page text still alerts — and the excerpt then says *the extracted page text did not move*, and explicitly declines to say the document is unchanged.

The second version that would have belonged here is a **best-effort** extractor. A document whose fonts are three-quarters mappable would yield three-quarters of a form, rendered as a confident text diff, with the changed quarter absent — and a reviewer reads that as *nothing changed here*. This is §A's own sentence with more moving parts: an extractor is a normalizer, and a normalizer that hides real text can hide a real change. So the subset is small and every document outside it is refused **whole**, by name, and falls back to the honest "the bytes changed, go and look" that cost the reviewer nothing they were not already paying. `sentinel sources check` prints each PDF source's outcome, so the documents this subset cannot read are a *counted* population rather than an assumed one — which is what would make a decision to widen it evidence-led rather than optimistic.
...
- **It is structural, not editorial.** `publish()` **requires** the registry, so there is no code path that can write an artifact without the thing that knows each source's status, and a merge-blocking gate (`-m source_labeling`, stage 6, alongside the no-unreviewed-in-feed gate it mirrors) asserts on the **published bytes** that no source appears in any artifact without it.
...
- **No tracking of any kind in the feeds and data files:** no analytics, no tracking pixel, no beacon, no UTM parameters, no third-party host in `feed.xml`, `changes.json`, `sources.json`, `status.json` or any per-jurisdiction file. `test_the_feed_requires_no_account_and_carries_no_tracking` and the sweep over every published artifact assert the published bytes contain none of these, and both run in the merge-blocking `feed_integrity` gate.
- **The two web pages make one third-party request, and only one: Google Analytics 4 (changed 2026-09-18, [ADR 0004](./adr/0004-count-page-visits-with-ga4.md)).** Until that date the published site made no third-party request at all — no CDN script, no external stylesheet, no web font, no image, no iframe, no JavaScript — because an HTML page is where this promise dies: a stylesheet from a CDN is not "just a stylesheet", it is a request that tells that CDN who is reading about trans ID law, in a country where that list is a targeting artifact. On 2026-09-18 the owner decided to count visits to `index.html` and `privacy.html` with GA4, as on her other public sites. That decision accepts part of the risk this section names: a record, held by Google, of browsers that read these pages. The limits that come with it are in ADR 0004 and on [`privacy.html`](https://chelseakr.github.io/id-churn-sentinel/privacy.html): production host only; never under Global Privacy Control or Do Not Track or after the footer opt-out; ad storage, ad personalization and Google signals off; no analytics cookies in the EEA, the UK and Switzerland; a page address sent without its query string or fragment, a referrer cut to its origin, and no custom event or user identifier. Everything else is still refused: the CSS is inline, there are no images, and the loader is the only script. The gate (`tests/test_site.py`) allows exactly that loader, byte for byte, and still fails on any other script, font, image, frame or tracker. `tests/test_analytics.py` runs the loader to prove the guards.
- **We do not want to know who reads the feeds.** Consequently, we cannot report feed readership, and we will never be able to. That is a deliberate trade: the metric is worth less than the risk. The web pages are now counted in aggregate by GA4 (see above); the feeds, which are what an organization integrates, are not.

**Honest limit, and it is now concrete rather than hypothetical: the feed is hosted on GitHub** — canonically on `chelseakr.github.io`, with `raw.githubusercontent.com` as a mirror. **GitHub's access logs exist and contain the IP address of every fetch**, including *which per-jurisdiction feed* was fetched, which is more identifying than the unscoped one rather than less. We do not control those logs, we do not receive them, and we cannot delete them. What we control is that they are not *ours*, are not enriched with any identity, and are not required to consume anything — there is no account to tie an IP to, because there is no account. A consumer who wants no trace can fetch over Tor or a VPN, or mirror the artifacts and serve them internally (AGPL-3.0-or-later, and explicitly fine); nothing about the feed makes any of that harder.

This is also why the published site made **no third-party request at all** until 2026-09-18: the hosting logs are a risk we cannot remove, so we refused to *add* a second one. A CDN font on the site would hand a third party the same list, for a nicer typeface. ADR 0004 is the one exception, decided by the owner and bounded as described above; nothing else has been added, and the gate keeps it that way.

**Status:** enforced, merge-blocking, as amended by ADR 0004 (2026-09-18). Residual risks named: host access logs, outside our control; and Google Analytics 4 on the two web pages, accepted by the owner, which reaches Google for readers who do not send GPC or DNT and have not opted out.
...
- **robots.txt is honored** — fetched per host, cached, and obeyed without appeal. A robots.txt that loads and disallows us wins; we do not fetch the page. (A robots.txt that is missing or unreachable is treated as permissive, the standard crawler posture.)
...
**What this actually cost us, stated rather than implied.** Honoring these rules is why the registry has **8 named gaps** instead of 8 quietly-spoofed sources. `oscn.net` publishes `Disallow: /` — it forbids all automated access to the entire site, and it is where Oklahoma publishes both its courts *and* its statutes, so Oklahoma's name-change process is unwatchable, full stop. `courts.michigan.gov` did the same, site-wide, as of 2026-08-21 — its SCAO name-change form is watched no more. `dmv.vermont.gov`, `dmv.colorado.gov` and `cdphe.colorado.gov` serve a browser and 403 us. Several hosts present TLS chains our trust store cannot verify. In every one of these cases there is an obvious two-line change — spoof a Chrome User-Agent, pass `verify=False`, ignore the robots.txt — that would "fix" coverage today. **We do not make it, and the gap is recorded instead.** A tool that lies about who it is, to a government server, on behalf of a population under surveillance, has not earned the trust it is asking for. (Some of these same-shaped refusals *were* closed rather than gapped, the same day: a different government host on the same domain — never a guessed replacement authority — answered honestly where the first one did not. Alaska, Arkansas, DC, Louisiana and South Dakota moved from gap to source this way; see the README's "Closing the map" section.)

**And the honest counterpoint, because the paragraph above is the easy half.** Refusing to spoof is not the same as refusing to *try*. Michigan and New Hampshire were absent entirely for exactly one reason — `michigan.gov` and every `nh.gov` host 403 our User-Agent — and "we honor their block" is a complete moral defense and an *incomplete engineering one*. A state almost always publishes the same policy content on a **second official surface**: its statutes, its administrative code, a court's PDF form. Those surfaces answer us honestly, and watching one is not a workaround — it is a different, equally official page for the same document class, and the entry says exactly what it is. Sixteen jurisdictions were closed that way, MI and NH among them, without sending a single dishonest byte.
...
**On TLS specifically.** Most of the broken-chain hosts here are not serving *invalid* certificates; they are serving **incomplete chains**, omitting an intermediate that a browser silently repairs by chasing the AIA extension. That chain *can* be completed correctly — by the server, which is whose job it is. We do not fetch the missing intermediate ourselves and we do not disable verification, and the reason is not pedantry: the population this tool serves is one for whom a silently-downgraded TLS connection to a government host is exactly the wrong thing to normalize. The defect is the server's; the gap is ours to record.
...
**What we do.** The feed reports human-reviewed machine observations about registry-claimed government-source candidates; a URL has source-authority standing in this contract only while its named verification is in date. It does not determine that an advocacy page is stale, audit or grade an organization, or state what changed legally. Namesake already operates a material daily canonical-PDF monitoring slice, so the analysis compared incumbent approaches with the narrower combined-contract hypothesis in [`CONSUMERS.md`](./CONSUMERS.md) and [`16-RESEARCH-SOURCES.md`](./16-RESEARCH-SOURCES.md). That comparison does not represent a proposal to, or a relationship with, any named organization.
...
**The risk.** The human-in-the-loop gate is load-bearing, and today the human is one person. A single reviewer who is unavailable, overloaded, or burned out becomes a bottleneck; a single reviewer who starts rubber-stamping turns the whole gate into theater while every test stays green.
