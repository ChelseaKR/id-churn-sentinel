...
- **Primary users — and this is the unusual part — are institutions, not individual guidance-seekers.** A4TE, Trans Lifeline, Namesake, and legal-aid organizations illustrate the domain; parts of the system may be better served by reusing or contributing to their work. Reuse/contribute/build evidence—not an assumption that they lack monitoring—decides that. None is affiliated with this project. See [`CONSUMERS.md`](./CONSUMERS.md).
...
- **The corrected finding.** The initial “zero machine-checkable change detection” premise did not survive first-party prior-art review: Namesake already operates a material daily canonical-PDF monitoring slice. Current work therefore tests the narrower combined-contract hypothesis and must reuse or contribute upstream when that is safer and cheaper than parallel implementation. See [`16-RESEARCH-SOURCES.md`](./16-RESEARCH-SOURCES.md).
...
**Shipped 2026-08-01 — a normalizer version change is not drift, and it is not "no change" either:**

- **The detector no longer compares hashes across normalizer versions.** A hash means nothing except relative to the normalizer that produced it, and `detect.py` compared them anyway — so bumping the normalizer turned every affected page into a change record whose diff was an artifact the tool had manufactured about itself and handed to a reviewer as drift. Bumping to `passage-text-v2` (for the end-tag fix, found by CodeQL) made that live: v1 baselines now sit in operators' stores alongside a v2 normalizer.
  **The fix is neither of the two obvious ones.** *Refusing* the comparison — the response to a corrected registry URL, above — fails unsafe here: it would blind every source with a v1 baseline for a whole pass, and a wrong "no change" about a government page that may have been scrubbed is the failure RESPONSIBLE-TECH §A is written about. *Labelling* it fails differently: one flagged alarm per source, and a caveat attached to a wall of alarms is a caveat that gets scrolled past. Neither is necessary, because unlike a corrected URL — where the old evidence is about a different page and is genuinely gone — a version bump leaves the evidence intact. **The store retains the baseline's bytes**, so the detector re-derives the baseline under today's contract and compares like for like. A version bump is then structurally incapable of reporting drift *and* incapable of hiding it: a page that really moved in the same pass still produces a change record, with a diff that is content rather than normalization. The first v2 pass over a v1 corpus prints one grouped block naming the transition instead of one alarm per source.
- **`sentinel baseline check` answers the same problem differently, and says why.** The committed baseline file holds hashes and no bytes, so it structurally cannot re-derive. Refusing every pre-v2 hash would leave a clean checkout unable to say anything at all, which is the hole that file exists to fill — so it *labels*: the MOVED line still appears, carrying the normalizer its committed hash was taken under, on the affected sources only. Baseline entries now record that pair; an entry written before the field loads as `unrecorded` rather than being assumed to be v1.
...
**Shipped (2026-07-13) — the consumable surface.** The thesis of `docs/CONSUMERS.md` is that the consumers are the incumbents; the embarrassing fact was that **they could not actually consume anything.** The artifacts lived in a git repo with no URL that served them, the source inventory existed only as a file a Python program parses, and the gaps were a paragraph. So:
...
### M5 — Consumers
The point of the whole thing. Success is not praise; success is **one organization wiring the feed into its content workflow**, on its own initiative and its own terms (`docs/CONSUMERS.md` is the integrator guide). The prerequisite that was quietly missing until M4 shipped: **there was nothing they could have wired.**
