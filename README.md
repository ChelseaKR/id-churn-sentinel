...
**Current alpha limits:** PDF and other binary changes do not yet have extracted-text or passage diffs; the SQLite store keeps only the newest **five** snapshots per source and prunes older ones; and neither durable long-term reproducibility nor full WCAG 2.2 AA conformance is a current claim. Both are V1 release gates. Iteration 1 now includes the canonical, fail-closed source-eligibility evaluator and `sentinel sources eligibility` denominator report. It intentionally does **not** yet switch the watcher or publisher to that predicate: the committed registry still needs human verification evidence/expiry and dated robots/terms/fetch-policy decisions, and no decision is inferred to make a dashboard look green. Wiring both call sites to the shared predicate is the next implementation iteration.
