...
The release issue must contain one row for each P0 ID: `SRC-01`, `SRC-02`, `DET-01`, `DET-02`, `REV-01`, `REV-02`, `PUB-01`, `PUB-02`, `PRIV-01`, `OPS-01`, `ACC-01`, `PILOT-01`, `PDF-01`, and `I18N-01`. Each row links the accepted backlog item, automated test run where applicable, human evaluation/operational receipt, domain attester, and residual-risk decision. The domain checklist below cannot substitute for this one-to-one index.

## Product and pilot — must pass

- [ ] V1 scope/non-goals match PRD, architecture, public copy, and support scripts.
- [ ] Two design partners completed at least five of six pilot cycles.
- [ ] Each partner completed its frozen eight-investigation matched set with no post-hoc exclusion; live-slot substitutions followed the sealed reserve order; real and exercise results are stratified; each partner independently achieved median paired research-time reduction ≥50%, never pooled.
- [ ] Each partner completed the same five fixed comprehension tasks twice and answered at least 8 of exactly 10 attempts correctly; missed attempts remain in the denominator.
- [ ] At least one partner has a named owner and standing post-pilot integration.
- [ ] A payer, committed sponsor, or approved runway for 12 months of safe operation is documented.
- [ ] Stop/pivot criteria were reviewed, including negative findings and substitutes.
...
- [ ] Funding/conflict disclosures are current; no partner has editorial control.
...
- [ ] Domain/certificate/host renewal, cost alerts, and 12-month operational budget are assigned.
...
- [ ] Integrator docs include schemas, examples, versioning, status, gaps, corrections, and support targets.
- [ ] Support staff can explain product evidence without giving legal advice.
- [ ] Security, privacy, correction, accessibility, and general support contacts are monitored.
- [ ] Reproducible competitor protocol, dated queries, first-party repository/operations evidence, candidates, unknowns, and the bounded combined-contract positioning were rechecked within 30 days; Namesake's daily canonical-source PDF extracted-text/line-diff and issue workflow received an explicit build/partner/buy disposition.
