...
2. **The published site and feed** — `index.html`, `privacy.html`, `feed.xml`, `changes.json`, `sources.json`, `status.json`, and one feed per jurisdiction. These are **generated** by `make publish` (`sentinel publish --out docs/`) and **committed**.
...
Do not hand-edit the generated files. `make publish` overwrites them, and a hand-edit to a published feed is a change no human reviewed reaching an organization that acts on it — the exact failure this project is built to refuse (`make no-unreviewed-in-feed` asserts the committed `changes.json` holds the property).

| Generated by `make publish` | Written by a human |
|---|---|
| `index.html` — the site, and the Pages entry point | `README.md` — this file |
| `privacy.html` — what reading the pages and feeds sends, and to whom ([ADR 0004](./adr/0004-count-page-visits-with-ga4.md)) | |
