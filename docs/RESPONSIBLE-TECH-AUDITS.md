...
**Honest limit, and it is now concrete rather than hypothetical: the feed is hosted on GitHub** — `raw.githubusercontent.com` today, `chelseakr.github.io` once Pages is enabled. **GitHub's access logs exist and contain the IP address of every fetch**, including *which per-jurisdiction feed* was fetched, which is more identifying than the unscoped one rather than less. We do not control those logs, we do not receive them, and we cannot delete them. What we control is that they are not *ours*, are not enriched with any identity, and are not required to consume anything — there is no account to tie an IP to, because there is no account. A consumer who wants no trace can fetch over Tor or a VPN, or mirror the artifacts and serve them internally (MIT, and explicitly fine); nothing about the feed makes any of that harder.
