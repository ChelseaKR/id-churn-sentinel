...
def test_feed_xml_is_well_formed_rss(confirmed_change: ChangeRecord) -> None:
    root = ET.fromstring(  # noqa: S314 — our own output
        feed_xml([confirmed_change], feed_url="https://example.org", generated_at=GENERATED)
    )
    ...
    # of (or into) their pipeline without parsing the description.
...
def test_feed_channel_metadata_names_the_disclaimer(confirmed_change: ChangeRecord) -> None:
    root = ET.fromstring(  # noqa: S314 — our own output
        feed_xml([confirmed_change], feed_url="https://example.org", generated_at=GENERATED)
    )
    channel = root.find("channel")
...
def test_xml_special_characters_are_escaped(confirmed_change: ChangeRecord) -> None:
    ...
    xml = feed_xml([nasty], feed_url="https://example.org", generated_at=GENERATED)
...
def test_publish_writes_both_artifacts(tmp_path: Path, confirmed_change: ChangeRecord) -> None:
    result = publish([confirmed_change], tmp_path / "dist", now=GENERATED)
...
def test_publish_creates_the_output_directory(
    ...
    publish([confirmed_change], out, now=GENERATED)
    assert (out / "feed.xml").exists()


def test_changes_json_shape(tmp_path: Path, confirmed_change: ChangeRecord) -> None:
    publish([confirmed_change], tmp_path, feed_url="https://example.org", now=GENERATED)
...
def test_items_are_newest_first_and_stable(tmp_path: Path, confirmed_change: ChangeRecord) -> None:
    ...
    publish([older, newer], tmp_path, now=GENERATED)
    payload = json.loads((tmp_path / "changes.json").read_text())

    assert [c["id"] for c in payload["changes"]] == ["newer0000000000", "older0000000000"]


def test_an_empty_feed_is_still_valid(tmp_path: Path) -> None:
    ...
    result = publish([], tmp_path, now=GENERATED)
...
def test_an_empty_feed_is_valid_rss_and_says_it_is_empty_rather_than_broken() -> None:
    ...
    xml = feed_xml([], feed_url="https://example.org", generated_at=GENERATED)
...
def test_publishing_nothing_writes_both_artifacts(tmp_path: Path) -> None:
...
def test_each_jurisdiction_feed_carries_only_its_own_items(
    ...
    publish([confirmed_change, arizona], tmp_path)
...
def test_a_scoped_feed_says_it_is_scoped(tmp_path: Path, confirmed_change: ChangeRecord) -> None:
...
def test_an_unreviewed_record_reaches_no_jurisdiction_feed(
    ...
    publish([observed_change], tmp_path)

    assert not (tmp_path / "feed-us-tx.xml").exists()  # no published record, no jurisdiction
