...
- **We do not want to know who reads the feeds.** Consequently, we cannot report feed readership, and we will never be able to. That is a deliberate trade: the metric is worth less than the risk. The web pages are now counted in aggregate by GA4 (see above); the feeds, which are what an organisation integrates, are not.
