...
- **12 of the 156 registered sources cannot currently be fetched** by our own crawler, every one of them confirmed by a live fetch on 2026-08-22 rather than inferred: `ssa.gov` (×2), `nycourts.gov`, `health.ny.gov`, `cdph.ca.gov`, `ilsos.gov`, and — as a regression found 2026-08-21 and reconfirmed 2026-08-22 — `travel.state.gov` (×2, now behind a Cloudflare bot-challenge), `ldh.la.gov` (×2, now a blanket 403 including on `/robots.txt` itself), and `legislature.mi.gov`'s two MCL PDFs (an incomplete TLS chain this tool's fetcher — unlike a browser — will not repair by chasing the AIA extension). All twelve are *watched in name only*. **They do not all carry the same evidence, and the difference matters:** the original six have **no baseline hash at all**, because a hash we did not observe is not a hash; the six that regressed *after* the 2026-07-13 baseline still hold the last hash we really did observe, which is a dated historical fact and not a claim about today. A run that cannot re-fetch any of them reports it as **unreachable, never as unchanged** — carrying the old hash forward is how an outage is kept from becoming drift, and it is never how an outage becomes a clean bill of health. They are not deleted: deleting them would erase the fact that we cannot watch them. (`nycourts.gov`'s name-change page also moved: its 2026-07-13 URL now returns 404 and the entry points at the real replacement path — but **this tool still cannot read that replacement either**, which answered 403 on all 5 attempts on 2026-08-22, 4 of them spaced two minutes apart. `www.nycourts.gov` refuses our User-Agent host-wide, `/robots.txt` itself included, so the move is a URL correction and not a fix. See issue #10 and the entry's own notes.) (A related but structurally different regression closed in #35: Michigan's SCAO name-change form moved from *unreachable* to a proper **named gap**, `robots-disallowed` — its host now disallows crawling site-wide, and CLAUDE.md's guardrail on robots.txt is to remove a source rather than keep a dead entry.)
