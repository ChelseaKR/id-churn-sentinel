...
- `ENG-01`: the ordered, checksummed migration framework and run/attempt entities are
  implemented. Remaining V1 review, correction, publication, retention, and backup entities plus
  a production backup/rollback rehearsal keep the item open.
- `ENG-02`: exact registry revision and eligible/attempted/successful source sets, persisted
  attempts, and constrained quiet/complete/partial/failed receipts are implemented. Deployment,
  alerting, and qualifying operational receipts remain open.
- `ENG-03`: closed-schema `status.json`, derived stale health, separate last-attempted and
  last-successful runs, and site health that does not reuse generation time are implemented.
  Staging/production evidence remains open.
