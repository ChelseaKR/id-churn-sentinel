...
- **11 of the 156 registered sources cannot currently be fetched**, and are therefore *watched in name only*: `ssa.gov` (both federal SSN sources) return 403 to every client we have; `health.ny.gov` returns 200 to a browser and 403 to our User-Agent; `cdph.ca.gov` ships a broken TLS chain; `ilsos.gov` will not complete an HTTP exchange at all; and, as a 2026-08-21 regression, `travel.state.gov` (both federal passport sources) now sits behind a Cloudflare bot-challenge, `ldh.la.gov` (both Louisiana vital-records sources) now returns a blanket 403 including on its own `/robots.txt`, and `legislature.mi.gov`'s two MCL PDFs now fail TLS verification (an incomplete chain a browser silently repairs and this fetcher will not). Each is named in `sources/registry.json` with the machine facts, and each carries **no baseline hash** — a hash we did not observe is not a hash. They are not deleted, because deleting them would erase the fact that we cannot watch them. (`nycourts.gov` genuinely moved rather than merely regressing — the 2026-08-21 pass found and fixed its real replacement URL, so it is off this list; see issue #10.)
...
**What this actually cost us, stated rather than implied.** Honouring these rules is why the registry has **8 named gaps** instead of 8 quietly-spoofed sources. `oscn.net` publishes `Disallow: /` — it forbids all automated access to the entire site, and it is where Oklahoma publishes both its courts *and* its statutes, so Oklahoma's name-change process is unwatchable, full stop. `courts.michigan.gov` tightened its own robots.txt to the same site-wide `Disallow: /` between 2026-07-13 and 2026-08-21, silently taking Michigan's SCAO name-change form down with it — honoured the same way, which is why that source was removed rather than kept as a dead entry (see the README's "Honest limits"). `dmv.vermont.gov`, `dmv.colorado.gov` and `cdphe.colorado.gov` serve a browser and 403 us. Two hosts present TLS chains our trust store cannot verify. In every one of these cases there is an obvious two-line change — spoof a Chrome User-Agent, pass `verify=False`, ignore the robots.txt — that would "fix" coverage today. **We do not make it, and the gap is recorded instead.** (A 2026-08-21 pass DID close five gaps this way — legitimately, on a different government host: `dccourts.gov` and `dmv.alaska.gov` blocked us and no longer name a gap, because DC's own Code Council site and Alaska's own Legislature site answer honestly for the same statute.) A tool that lies about who it is, to a government server, on behalf of a population under surveillance, has not earned the trust it is asking for.
