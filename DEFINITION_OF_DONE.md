...
- The watcher and publisher call the same dated eligibility predicate without an operator bypass. Every watch persists the exact eligible, attempted, and successful source-ID sets and registry revision. Fetch failure stays visible in an eligible run's attempt denominator and does not silently improve coverage; publication refuses an observation backed by an unverified, expired, denied, rejected, withdrawn, missing, or identity-mismatched source.
- Public run health comes only from persisted run receipts. `generated_at` is never treated as watch success; `status.json` and the site distinguish running, quiet, complete, partial, failed, and stale states and keep the last attempted run separate from the last successful run.
