...
- the trust of trans people and downstream organizations.
...
- a hostile scraper attempting to infer readers or downstream organizations;
...
Store only names necessary for accountable review, work contact details, role, and decisions. Limit access to the service owner and governance lead.
...
- The offline signing-key runbook covers generation, two-person activation, encrypted custody, rotation overlap, verifier distribution, revocation, compromise recovery, and consumer notification. The crawler and pull-request CI never receive signing material.
...
- **Sev-3:** isolated source failure, accessibility regression without blocked critical path, or downstream integration break. Acknowledge next business day.

Every Sev-1/2 produces a preserved timeline, containment record, consumer notification decision, correction/withdrawal when needed, and blameless postmortem. Evidence integrity takes priority over uptime.
