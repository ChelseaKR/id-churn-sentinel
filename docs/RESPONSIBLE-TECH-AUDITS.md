...
- **12 of the 156 registered sources cannot currently be fetched**, and are therefore *watched in name only*: `ssa.gov` (both federal SSN sources) returns 403 to every client we have; `health.ny.gov` returns 200 to a browser and 403 to our User-Agent; `cdph.ca.gov` ships a broken TLS chain; `ilsos.gov` will not complete an HTTP exchange at all; `nycourts.gov`'s name-change page moved — its old URL now 404s and the entry points at the replacement path, which this tool still cannot read (HTTP 403 on all 5 attempts on 2026-08-22, 4 of them spaced two minutes apart; the host refuses our User-Agent site-wide, `/robots.txt` included); and, as a regression found 2026-08-21 and reconfirmed by live fetch on 2026-08-22, `travel.state.gov` (both federal passport sources) now sits behind a Cloudflare bot-challenge, `ldh.la.gov` (both Louisiana vital-records sources) now returns a blanket 403 including on its own `/robots.txt`, and `legislature.mi.gov`'s two MCL PDFs now fail TLS verification (an incomplete chain a browser silently repairs and this fetcher will not). Each is named in `sources/registry.json` with the machine facts. **The evidence behind them is not uniform, and saying so is the point:** the six unreachable at seeding carry **no baseline hash** — a hash we did not observe is not a hash — while the six that regressed after the 2026-07-13 baseline still hold the last hash actually observed, dated to when it was observed. Neither kind is ever reported as *unchanged*: a fetch that fails is reported as a failure on every run it recurs, because "we could not look" and "we looked and nothing moved" are the two sentences this project exists to keep apart. They are not deleted, because deleting them would erase the fact that we cannot watch them.
